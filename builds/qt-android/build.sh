...
##
# Build wallet from local source

(android_build_verify_so "libwap.so" "libczmq.so" &> /dev/null) || {
    rm -rf "${cache}/wallet"
    (cp -r ../.. "${cache}/wallet" && cd "${cache}/wallet" \
        && make clean && rm configure config.status)
    rm 
    
    export LIBTOOL_EXTRA_LDFLAGS='-avoid-version'
    
    (cd "${cache}/wallet" && ./autogen.sh \
...
android_build_verify_so "libwap.so" "libczmq.so"
